...
# Users Authentication Function =======================================================================================================================================================================================================================
	...
	def gotoWorkspace(self):
		...
		# Database Connections and Tables Creations
		...
		c.execute("INSERT INTO sessions VALUES (:user, :login_dt, :logout_dt)",
			{
			'user': username.get(),
			'login_dt': now,
			'logout_dt': ' '
		...
		c.execute("SELECT *, oid from sessions WHERE login_dt = ?", (now,))

		records = c.fetchall()

		for record in records:
			sid.insert(0, record[3])
...
class Workspace():
	...
	def __init__(self, master):
		...
		sid = Entry(self.master, width = 5, bd = 0, fg = "#ffffff")
		sid.grid(row = 1, column = 2)
		...
		maintenanceBtn = Button(self.bottomPanel, text = "Admin Panel", command = self.gotoMaintenance, width = 15, bd = 1, relief = "ridge" , font = ('Helvetica', 10, 'bold'))
...
class Maintenance():
	...
	def __init__(self, master):
		...
		self.master = master
		self.master.geometry('250x150+33+500')
		...
		def sessions():
			...
			sessions = Toplevel()
			sessions.geometry('600x450+700+0')
			sessions.title('KMS - Sessions')
			sessions.iconbitmap('kits.ico')
			sessions.configure(background = '#ffffff')


			mainFrame = Frame(sessions, pady = 20, padx = 20)
			mainFrame.grid(row = 0, column = 0, pady = 20, padx = 20)


			dataList = Listbox(mainFrame, font = ('Helvetica', 9, 'bold'), width = 70, height = 20, bg = "#ffffff")
			dataList.grid(row = 0, rowspan = 15, column = 0)

			scrollbarY = Scrollbar(mainFrame, command = dataList.yview)
			scrollbarY.grid(row = 0, rowspan = 15, column = 1, sticky = 'ns')
			scrollbarX = Scrollbar(mainFrame, orient = 'horizontal' , command = dataList.xview)
			scrollbarX.grid(row = 16, column = 0, sticky = 'we')
			dataList.configure(yscrollcommand = scrollbarY.set, xscrollcommand = scrollbarX.set)
			...
			c.execute("SELECT *, oid from sessions")
			records = c.fetchall()
			
			dataList.delete(0, END)

			dataList.insert(END, "USER SESSIONS")
			dataList.insert(END, "\n" + "\n")
			dataList.insert(END, "Id." + "       " + "User" + "                           " + "Login DT" + "                                     " + "Logout DT")
			dataList.insert(END, "\n" + "\n")

			for record in records:
				dataList.insert(END, str(record[3])+ "       " + str(record[0]) + "       " + str(record[1]) + "       " + str(record[2]) + "\n")
		...
		self.sessionsBtn = Button(self.master, text = "Sessions", command = sessions, bd = 2, width = 10, font = ('Helvetica', 10, 'bold'), bg = "#ffffff", fg = "#000000", relief = RIDGE)
		self.sessionsBtn.grid(row = 1, column = 1)
